...
class Frame extends Component {
  ...
  newFrame = () => {
    ...
    if (this.props.projectId !== null) {
      ProjectFetches.fetchCreateFrame({
  ...
  deleteFrame = id => {
    ProjectFetches.fetchDeleteFrame(id).then(res => {
      this.props.deleteFrame(id);
  ...
  drawGhost = () => {
    this.props.ghostContext.clearRect(
      0,
      0,
      this.props.canvasWidth,
      this.props.canvasHeight
  ...
  bringCanvasToFront = id => {
    ...
    tmpImg.onload = () => {
      ...
      this.props.previewContext.clearRect(
        0,
        0,
        this.props.canvasWidth,
        this.props.canvasHeight
      );
      this.props.previewContext.drawImage(
        tmpImg,
        0,
        0,
  ...
  render() {
    return (
      <div style={FrameStyles.FramesContainer}>
        {this.renderFrames()}
        <Button icon onClick={this.newFrame} style={FrameStyles.newFrameButton}>
          <Icon name="add" size="huge" style={{ verticalAlign: "middle" }} />
        </Button>
      </div>
    );
  }
